...
"    call setqflist(qf_taglist)
"    botright  cwindow 4
    call <SID>FindMemberWindow(qf_taglist)
  endif
endfunction

function! s:FindMemberWindow(qf_taglist)
    let g:lastFindMemTagList=copy(a:qf_taglist)
    let g:xbn=bufname('%')
    if bufwinnr('__outline__')==-1
        vsp __outline__
        setlocal buftype=nofile
    endif

    syn match Comment "[^-]"
  for item in a:qf_taglist
      let defTypeDic=AnalysisDefind(item["text"])
"      echom "------"
"      echom "--filename:".item["filename"]
"      echom "--text:".item["text"]
"      echom "--lnum:".item["lnum"]
"      echom "--col:".item["col"]
"      echom "--vcol:".item["vcol"]
      silent call append(line('$'),<SID>GetOutLineStr(defTypeDic))
  endfor
  nnoremap o :silent call <SID>LocationSelection(g:lastFindMemTagList,getline('.'))<cr>
  nnoremap gd :silent call <SID>LocationSelection(g:lastFindMemTagList,getline('.'))<cr>
  nnoremap <Enter> :silent call <SID>LocationSelection(g:lastFindMemTagList,getline('.'))<cr>
  noremap <2-LeftMouse> :silent call <SID>LocationSelection(g:lastFindMemTagList,getline('.'))<cr>

"  autocmd! Bufleave :setlocal modifiable|:close
...
function! s:GetOutLineStr(defTypeDic)
    let str=a:defTypeDic["type"]."\t\t".a:defTypeDic["defName"]
    return str
endfunction

"get hideType,returnType,defName,classType,baseClassType
function! AnalysisDefind(txt)
    let dicDefine={}
    let dicDefine["txt"]=a:txt
    let dicDefine["errorMsg"]="none"
"    echom '[ana] txt:'.a:txt
    let lsp=split(a:txt," ")
    let listLen=len(lsp)
"    echom '[ana] listLen:'.listLen
"    if match(a:txt,'\v.+\(.*\)')!=-1
"    echom '[ana] func txt:'.a:txt
"    else
"    echom '[ana] not func txt:'.a:txt
"    endif


    "include (),means function define
    if match(a:txt,'\v.+\(.*\)')!=-1
        let dicDefine["type"]='func'
"        echom '[ana] set type:func:'.a:txt
        "function define
        if match(a:txt,'\v.*(<private>|<public>|<protected>).+')!=-1
            let dicDefine["hideType"]=lsp[0]
            let dicDefine["retType"]=lsp[1]
            let defNameStartIndex=2
"            echom '[ana] has hide type,defname str:'.lsp[2]
        else
            let dicDefine["hideType"]='private'
            let dicDefine["retType"]=lsp[0]
"            echom '[ana] not has hide type,defname str:'.lsp[1]
            let defNameStartIndex=1
        endif

"        echom '[ana] defNameStartIndex:'.defNameStartIndex.',listLen-1:'.(listLen-1)
        if defNameStartIndex==(listLen-1)
            let dicDefine["defName"]=lsp[-1]
"            echom '[ana] == :'.dicDefine["defName"]
        else
            let dicDefine["defName"]=join(lsp[defNameStartIndex:(listLen-1)],' ')
"            echom '[ana] != :'.dicDefine["defName"]
        endif

    "include class,means class define
    elseif match(a:txt,'\v.*class.+')!=-1
        let dicDefine["type"]='class'
"        echom '[ana] set type:class'
        "class define
        let classIndex=index(lsp,'class')

        "default hideType
        if classIndex==0
            let dicDefine["hideType"]='private'
        else
            let dicDefine["hideType"]=lsp[classIndex-1]
        endif

            "### get class name and baseClassname if has
            let cNames=lsp[classIndex+1]
            "has baseClass
            if match(cNames,':')!=-1
                let cSplist=split(cNames,':')
                let dicDefine["defName"]=cSplist[0]
                let dicDefine["baseClass"]=cSplist[1]
            else
                let dicDefine["defName"]=cNames
            endif
   " property define 
    else
        let dicDefine["type"]='property'
"        echom '[ana] set type:property'
        "has hide type
        if match(a:txt,'\v.*(<private>|<public>|<protected>).+')!=-1
            let dicDefine["hideType"]=lsp[0]
        else
...
function! s:LocationSelection(full_list,lineContent)
    for item in a:full_list
        let defTypeDic=AnalysisDefind(item["text"])
        let linestr=<SID>GetOutLineStr(defTypeDic)
        if linestr==a:lineContent
            let nr=bufwinnr(bufname(g:xbn))
            ":help wincmd
            "use ctrl-w,jump to another splited window
            "then the focused window is the target window
            "so the outine window whill not close
            exe nr.'wincmd w'
            call cursor(item["lnum"],item["col"])
            "add by cyc. cursor move to screen center
            normal! zz
            "delete __outline__ buffer
            exe 'bd #'
"            silent call OmniSharp#JumpToLocation(item["filename"],item["lnum"],item["col"])
            let g:xbn=bufname('%')
            break
        endif
    endfor
...
"function! s:LocationSelection()
"    echo "stoc tag:".a:tag
"    exe 'match Todo /%'.line('.').'1/'
"    let nr=bufwinnr(bufname(g:xbn))
"    exe nr.'wincmd w'
...
function! OmniSharp#ServerIsRunning() abort
    "cyc changed;ycm Omnisharp will run at first
    let g:serverSeenRunning = 1
    return 1
"  try
