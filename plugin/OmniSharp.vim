...
"    let g:OmniSharp_server_path = join([expand('<sfile>:p:h:h'), 'server', 'OmniSharp', 'bin', 'Debug', 'OmniSharp.exe'], '/')
    let g:OmniSharp_server_path = join([expand('<sfile>:p:h:h'), 'third_party','ycmd','third_party','OmniSharpServer', 'OmniSharp', 'bin', 'Release', 'OmniSharp.exe'], '/')
  else
    let g:OmniSharp_server_path = join([expand('<sfile>:p:h:h'), 'omnisharp-roslyn', 'artifacts', 'scripts', 'Omnisharp'], '/')
  endif
"  echom "Omnisharp_server path:".g:OmniSharp_server_path
endif
